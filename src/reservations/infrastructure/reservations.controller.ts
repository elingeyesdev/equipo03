...
export class ReservationsController {
  ...
  @ApiResponse({
    status: 400,
    description: 'Datos inválidos o targetUserId faltante (gerente)',
  })
  @ApiResponse({
    status: 403,
    description:
  ...
  @ApiResponse({
    status: 404,
    description: 'Horario o usuario objetivo no encontrado',
  })
  ...
  @Roles('INSTRUCTOR', 'ENTRENADOR', 'GERENTE', 'SUPER_ADMIN')
  @ApiOperation({
    summary: 'Walk-in: crea reserva COMPLETADA sin validación de tiempo',
  })
  @ApiBody({ type: WalkInDto })
  @ApiResponse({ status: 201 })
  async walkIn(@Body() body: WalkInDto) {
    const reservation = await this.svc.createWalkIn(
      body.userId,
      body.scheduleId,
    );
  ...
  @ApiQuery({
    ...
    example: 'CONFIRMADA',
    description:
      'Filtrar por estado exacto (CONFIRMADA|COMPLETADA|CANCELADA|PENDIENTE)',
  })
  @ApiQuery({
    name: 'page',
    required: false,
  ...
  @ApiQuery({
    ...
    required: false,
    example: 36,
    description: 'Filtrar por sede (gymId)',
  })
  @ApiResponse({
    status: 403,
    description: 'ENTRENADOR u otro rol sin permiso de listado',
  })
  findAll(
  ...
  @Get('me')
  @ApiOperation({
    summary: 'Mis reservas (userId del JWT — aislamiento estricto)',
  })
  ...
  findMine(@Request() req: any, @Query('date') date?: string) {
    ...
    if (!userId || isNaN(userId)) {
      throw new UnauthorizedException(
        'No se pudo extraer el ID del token. Vuelve a iniciar sesión.',
      );
    }
  ...
  @Get('user/:userId')
  @ApiOperation({
    summary: 'Reservas de usuario (GERENTE/SUPER_ADMIN pueden especificar ID)',
  })
  @ApiResponse({
    status: 403,
    description: 'CLIENTE solo puede ver las propias',
  })
  ...
  @ApiOperation({
    summary:
      'Auditoría de sede: reservas CONFIRMADA/COMPLETADA (GERENTE / SUPER_ADMIN)',
  })
  @ApiQuery({
    name: 'date',
    required: false,
  ...
  @ApiResponse({
    status: 200,
    description: 'Lista de reservas con usuario, actividad e instructor',
  })
  ...
  @ApiOperation({
    summary:
      'Obtener JWT efímero (3 min) para QR de check-in (solo dueño de la reserva)',
  })
  @ApiResponse({
    status: 200,
    description: '{ qrToken: string } — JWT que expira en 3 minutos',
  })
  ...
  @Roles('GERENTE', 'SUPER_ADMIN', 'RECEPCIONISTA')
  @ApiOperation({
    summary: '[DEPRECATED] Check-in por ID (usar /check-in/token)',
  })
  @ApiBody({ type: CheckInReservationDto })
  @ApiResponse({
    status: 200,
    description: 'Check-in registrado, reserva marcada COMPLETADA',
  })
  @ApiResponse({ status: 400, description: 'Reserva no está CONFIRMADA' })
  @ApiResponse({
    status: 403,
    description: 'Rol sin permiso o reserva de otra sucursal',
  })
  @ApiResponse({ status: 404, description: 'Reserva no encontrada' })
  ...
  @ApiOperation({
    summary:
      'Check-in por QR token — bloqueo pesimista, anti-fraude (GERENTE / SUPER_ADMIN)',
  })
  @ApiBody({ type: CheckInByTokenDto })
  @ApiResponse({
    status: 201,
    description: 'Check-in registrado, reserva COMPLETADA',
  })
  @ApiResponse({ status: 400, description: 'Reserva no está CONFIRMADA' })
  @ApiResponse({
    status: 403,
    description: 'QR de otra sucursal o rol sin permiso',
  })
  @ApiResponse({
    status: 404,
    description: 'QR inválido o reserva no encontrada',
  })
  ...
  @Roles('GERENTE', 'SUPER_ADMIN', 'RECEPCIONISTA')
  @ApiOperation({
    summary: 'Confirmar ingreso de una reserva (marca como COMPLETADA)',
  })
  @ApiResponse({ status: 200, description: 'Reserva actualizada a COMPLETADA' })
  @ApiResponse({
    status: 400,
    description: 'Reserva ya completada o cancelada',
  })
  @ApiResponse({
    status: 403,
    description: 'Rol sin permiso o de otra sucursal',
  })
  @ApiResponse({ status: 404, description: 'Reserva no encontrada' })
