...
export class MetricsController {
  ...
  @Post()
  @ApiOperation({ summary: 'Registrar medición física' })
  ...
  @Get('user/:userId')
  @ApiOperation({ summary: 'Historial de métricas de usuario' })
  findByUser(@Param('userId', ParseIntPipe) uid: number) {
    return this.svc.findByUser(uid);
  }

  @Get('user/:userId/latest')
  @ApiOperation({ summary: 'Última medición del usuario' })
  findLatest(@Param('userId', ParseIntPipe) uid: number) {
    return this.svc.findLatest(uid);
  }

  @Get(':id')
  @ApiOperation({ summary: 'Obtener métrica por ID' })
