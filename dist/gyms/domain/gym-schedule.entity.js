"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.GymSchedule = void 0;
const typeorm_1 = require("typeorm");
const gym_entity_1 = require("./gym.entity");
let GymSchedule = class GymSchedule {
    id;
    gymId;
    dayOfWeek;
    opensAt;
    closesAt;
    isHoliday;
    gym;
};
exports.GymSchedule = GymSchedule;
__decorate([
    (0, typeorm_1.PrimaryGeneratedColumn)(),
    __metadata("design:type", Number)
], GymSchedule.prototype, "id", void 0);
__decorate([
    (0, typeorm_1.Column)({ type: 'integer', name: 'gym_id' }),
    __metadata("design:type", Number)
], GymSchedule.prototype, "gymId", void 0);
__decorate([
    (0, typeorm_1.Column)({ type: 'varchar', length: 10, name: 'day_of_week' }),
    __metadata("design:type", String)
], GymSchedule.prototype, "dayOfWeek", void 0);
__decorate([
    (0, typeorm_1.Column)({ type: 'time', name: 'opens_at' }),
    __metadata("design:type", String)
], GymSchedule.prototype, "opensAt", void 0);
__decorate([
    (0, typeorm_1.Column)({ type: 'time', name: 'closes_at' }),
    __metadata("design:type", String)
], GymSchedule.prototype, "closesAt", void 0);
__decorate([
    (0, typeorm_1.Column)({ type: 'boolean', name: 'is_holiday', default: false }),
    __metadata("design:type", Boolean)
], GymSchedule.prototype, "isHoliday", void 0);
__decorate([
    (0, typeorm_1.ManyToOne)(() => gym_entity_1.Gym, (g) => g.schedules, { onDelete: 'CASCADE' }),
    (0, typeorm_1.JoinColumn)({ name: 'gym_id' }),
    __metadata("design:type", gym_entity_1.Gym)
], GymSchedule.prototype, "gym", void 0);
exports.GymSchedule = GymSchedule = __decorate([
    (0, typeorm_1.Entity)('gym_schedules')
], GymSchedule);
//# sourceMappingURL=gym-schedule.entity.js.map