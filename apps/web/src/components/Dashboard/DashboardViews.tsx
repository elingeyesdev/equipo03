  ...
  const handleSaveSede = async (formData: any) => {
    try {
      ...
      if (sedeToEdit) {
        ...
        // 2ª Petición: Actualizar ubicación si el formulario la tiene
        ...
        console.log(`[Final Contract]: Enviando payload a /gyms/${sedeToEdit.id}/location -> ${JSON.stringify(locationPayload)}`);
        await apiClient.put(`/gyms/${sedeToEdit.id}/location`, locationPayload).catch(async (e) => {
          console.warn('[Location] PUT falló, intentando POST:', e.message);
          await apiClient.post(`/gyms/${sedeToEdit.id}/location`, locationPayload).catch(err => {
            console.warn('[Location] No se pudo actualizar ubicación, continuará sin ella:', err.message);
          });
        });

        setGyms(prev => prev.map(g => g.id === sedeToEdit.id
          ? { ...g, name: formData.name, maxCapacity: formData.maxCapacity, isOpen: formData.isOpen, location: { address: formData.address, city: formData.city } }
          : g
        ));
      } else {
