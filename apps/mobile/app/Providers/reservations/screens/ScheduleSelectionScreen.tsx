...
export const ScheduleSelectionScreen = ({ route, navigation }: Props) => {
  ...
  // Hooks de lógica
  const { canReserve, isLoading: isCheckingSub } = useSubscriptionGuard();
  ...
  const handleConfirmReservation = async () => {
    ...
    try {
      ...
      navigation.navigate('ReservationSuccess', { 
        ...
        activityName: gymName // Idealmente pasaríamos el nombre de la actividad
      });
            ...
            renderItem={({ item }) => (
              <ScheduleSlot
                ...
                onSelect={() => {
                  ...
                  if (canReserve) setIsModalVisible(true);
                  else Alert.alert('Aviso', 'Ve a tu perfil para adquirir una membresía.');
                }}
              />
            )}
          />
